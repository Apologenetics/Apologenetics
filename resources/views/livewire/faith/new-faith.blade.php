<div class="p-8 bg-white rounded-2xl shadow-xl flex flex-col space-y-8">
    ...
    <div class="flex flex-row space-x-4 w-full">
        ...
        <div class="flex flex-col space-y-2 w-full">
            <x-label for="end_of_faith">{{ __('End of Faith') }}</x-label>
            <x-input type="text" wire:model.defer="state.end_of_faith" id="end_of_faith" />
            @error('state.end_of_faith')<span class="text-red-500 text-sm">{{ $message }}</span>@enderror
        </div>
        <!-- Column 2 -->
        <div class="flex flex-col space-y-2 w-full">
            <x-label for="reason_left">{{ __('Reason Left') }}</x-label>
            <x-input type="text" wire:model.defer="state.reason_left" id="reason_left" />
